    ...
    N: PartialOrd + Zero + Copy,
{
    ...
    /// + 算术运算，可拷贝，可偏序比较；
    /// + 实际使用的时候就是数字类型，比如：i8/u8/i32/u32/f32/f64；
    pub fn new(nodes_num: usize) -> AJPSStar<N> {
        let mut result = AJPSStar::<N> {
        ...
        Filter: AJPSFilter<N>,
    {
        ...
        while self.open.len() != 0 || !best_next.is_none() {
            ...
            for neighbor in map.get_neighbors(cur_node_idx, parent) {
                let mut parent_to_neighbor = false; // parent节点直接连到neighbor
                let mut parent_g = N::zero();
                if !parent.is_none() {
                    let parent = parent.unwrap();
                    if parent == neighbor {
                        continue;
                    }
                    if filter.is_pass(parent, neighbor) {
                        parent_to_neighbor = true;
                        parent_g = self.impl_nodes[parent].g;
                    }
                }
                ...
                if node.status != NodeStatus::None {
                    ...
                    if node.status == NodeStatus::Linked || node.status == NodeStatus::Opened {
                        g = {
                            if parent_to_neighbor {
                                parent_g + filter.get_g(neighbor, parent.unwrap())
                            } else {
                                cur_node_g + filter.get_g(neighbor, cur_node_idx)
                            }
                        };
                        if g > node.g {
                            continue;
                ...
                if parent_to_neighbor {
                    node.reset_parent(parent.unwrap(), g);
                } else {
...
impl<N: PartialOrd + Zero + Copy> ANodeImpl<N> {
    ...
    // 通过Node获取新的内部ANode
    fn init<Filter: AJPSFilter<N>>(&mut self, src: NodeIndex, end: NodeIndex, filter: &Filter) {
